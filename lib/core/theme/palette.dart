


import 'package:flutter/material.dart';

class Palette {
   static Color PRIMARY_DARK = Color(0xffEB9B00);
   static Color TEXT_DARK = Color(0xff253D4E);

   static Color grayTextLight = Color(0xff999EA1);
   static Color PRIMARY_BG = Color(0xffF4F4F4);
     static Color RED = Color(0xffFF0000);
    static Color BACKGROUND = Color(0xffFFFFFF);
    static Color LIGH_BACKGROUND = Color(0xffF5F4F4);
    static Color LIGH_BACKGROUND_GREEN = Color(0xffCDE7C1);
    static Color LIGH_BACKGROUND_GREEN2 = Color(0xffE0F3D8);

    static Color ACTIVE = Color(0xff03E500);
    static Color ACTIVE_BACKGROUND = Color(0xffECFFEC);

    static Color PRIMARY = Color(0xff01613B);
    static Color LIGHT_PRIMARY = Color(0xff73A892);
    static Color DARK_PRIMARY = Color(0xff144F29);
    static Color LIGHT_BACKGROUND = Color(0xffF2F2F2); 
    static Color ORANGE = Color(0xffE26B13); 
    static Color ORANGE_DARK = Color(0xffE35B31); 
    static Color DARKBROWN = Color(0xff4B190D); 
    static Color YELLOW = Color(0xffEC9B18); 
    static Color LIGHT_YELLOW = Color(0xffFFFF66); 


    static Color BLACK = Color(0xff161B14); 
    static Color BLACK_SIMI = Color(0xff1C1C1C); 
    static Color LIGHT_TEXT = Color(0xff999EA1); 

     static Color TEXT_LIGHT = Color(0xff8B8B8B);


static Color LIGH_BG2 = Color(0xffF4F4F4);
static Color lightBgTheme3 = Color(0xffF7F7F7);
static Color SHIMMER100 = Color(0xFFefefef);
static Color SHIMMER300 = Color(0xFFdcdcdc);

static Color LIGHT_TEXT_PRIMARY = Color(0xFFD7E6E0);
static Color LIGHT_RED = Color(0xFFFB714A);


static Color BG_DARK = Color(0xFF1C404B);
static Color  BG_LIGH_3 = Color(0xFFF2F5F7);


//shimmer
static Color  SHIMMER1 = Color(0xFFefefef);
static Color  SHIMMER2 = Color(0xFFefefef);
static Color  SHIMMER3 = Color(0xffF7F7F7);

static Color grayTextV2 = Color(0xff8B8B8B);



//FINAL COLOR
static Color  FBG = Color(0xffF4F4F4);
static Color  FBGPRIMARY = Color(0xff3EB075);

 static Color GREEN1 = Color(0xffE2F1EC);
    static Color GREEN2 = Color(0xff15A671);
    static Color GREEN3 = Color(0xff15A671);
    static Color GRAY1 = Color(0xffDADADA);
    static Color GREEN0 = Color(0xffDCF5D4);
    static Color FGREEN1 = Color(0xff0ba360);
    static Color FGREEN2 = Color(0xff3cba92);



    // new theme
    static Color  text =Colors.black;
    static Color  lightText = Color(0xff666666);
    static Color  orange = Color(0xffE97522);
    static Color  lightOrange = Color(0xffFFF5EE);
    static Color  bg = Color(0xffFBFBFB);
    // static Color  bg = Color(0xffF9F9F9);
    static Color  card1 = Color(0xff00A644);
    static Color  card2 = Color(0xff0CAF57);
    static Color  card3 = Color(0xff00431E);
}