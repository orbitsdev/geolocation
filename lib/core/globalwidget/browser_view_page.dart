...
class BrowserViewerPage extends StatefulWidget {
  ...
  @override
  State<BrowserViewerPage> createState() => _BrowserViewerPageState();
}

class _BrowserViewerPageState extends State<BrowserViewerPage> {
  final BrowserviewerController browserController = Get.put(BrowserviewerController());
...
void dispose() {
  ...
  String _getViewerUrl(String fileUrl) {
    ...
    } else if (fileUrl.endsWith('.docx') || fileUrl.endsWith('.xlsx') || fileUrl.endsWith('.pptx')) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(widget.file.file_name ?? 'Document'),
        leading: IconButton(
          onPressed: () async {
              ...
              builder: (controller) {
                return InAppWebView(
                  ...
                  onLoadHttpError: (webViewController, url, statusCode, description) {
                    controller.handleLoadError(
                      Failure(
                        message: "HTTP Error: $statusCode, Description: $description",
                      ),
                    );
                  },
                );
